...
// config_asic.vh
//
// AMOEBA 65 nm tapeout configuration.
//
// Derived from pkg/config_baremetal_linux.vh (fpga/linux_boot branch, the
// config that boots a soft-float Linux on the FPGA).  That file is kept
// verbatim next to this one so `diff` shows exactly what the silicon changes.
// Select with +define+AMOEBA_CONFIG_ASIC (see amoeba_config_select.vh); the
// build must also pass ECE411_NO_FLOAT, as for every FPU-less config.
//
// What this file changes relative to config_baremetal_linux.vh, and why:
//
//   PLIC_SUPPORTED   0 -> 1   Decode only.  amoeba_uncore never instantiates
//                             plic_apb; with the region decoded, the PMA marks
//                             it uncached/non-idempotent and the access leaves
//                             the chip over the link, so a PLIC can live on the
//                             FPGA without a respin.  Costs one address
//                             comparator.  Leave it out of the DTS and the
//                             kernel never touches it (UART polled, as today).
//   UART_SUPPORTED   1 (kept) Decode only, same mechanism.  The 16550 is on
//                             the FPGA when PERIPH_ONCHIP=0 (tapeout), on the
//                             die when PERIPH_ONCHIP=1 (area upper bound).
//   USE_SRAM         define   1 under +define+AMOEBA_USE_SRAM (synthesis), 0
//                             otherwise.  Run the regression once with it set
//                             before freeze; the netlist uses macros.
//   GPIO_SUPPORTED   0 (firm) Was "// Maybe".  GPIO would be 3x32 pads.
//
// What this file deliberately does NOT change, and the open item on each:
//
//   *_WAYSIZEINBYTES 2048     4 KiB/way is the measured-good geometry (2.57x
//                             faster Linux boot than 512 B) and the VIPT
//                             ceiling; 2 KiB is the area-pending value from
//                             the branch.  Decide once SRAM macro area is in.
//   ZICNTR/SSTC      0        Means the core never consumes MTIME_CLINT; the
//                             CLINT stays on-die anyway (fpga/asic/BRINGUP.md
//                             s2), it is ~2 k GE and keeps mtime = core cycles.
//   PMP_ENTRIES      0        No Keystone on the tapeout image.
//
// Everything below is config_baremetal_linux.vh with the edits above applied
// in place and marked "ASIC:".
///////////////////////////////////////////////////////////////////////////////
//
...
// ASIC: OPEN.  Revisit at 4096 once SRAM macro area is known; see file header.
...
localparam logic BOOTROM_SUPPORTED = 0;  // reset vector is in EXT_MEM; first fetch is over the link
...
localparam logic GPIO_SUPPORTED = 0;        // ASIC: firm; GPIO is 3x32 pads
...
// ASIC: decode only under PERIPH_ONCHIP=0 -- the 16550 sits on the FPGA
// behind the link, at this same address, so the DTS is unchanged.  Under
// PERIPH_ONCHIP=1 amoeba_uncore instantiates uart_apb here.
...
// ASIC: decode only.  amoeba_uncore folds HSELPLIC into the external port and
// never instantiates plic_apb, so this parameter buys the FPGA the option of
// hosting a PLIC at 0x0C00_0000 later, at the cost of one address comparator.
// The Linux DTS on this branch has no plic node and the UART polls; both keep
// working.  Setting this to 0 would make the region a PMA access fault and
// bake the no-PLIC decision into silicon.
...
// Consumed only by plic_apb, which is on the FPGA (PERIPH_ONCHIP=0) or on the
// die (PERIPH_ONCHIP=1); either way it elaborates against these.
...
// ASIC: macros for synthesis, behavioural for simulation.  The two must be
// regressed against each other at least once before freeze.
`ifdef AMOEBA_USE_SRAM
