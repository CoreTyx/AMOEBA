...
if check_tool_version $RISCV_GNU_TOOLCHAIN_VERSION; then
    ...
    for attempt in 1 2 3; do
        ...
        if grep -Eqi 'RPC failed|HTTP 502|HTTP 503|did not contain|unable to access' "$RISCV/logs/$STATUS.log"; then
            echo -e "${WARNING_COLOR}RISC-V GNU Toolchain build hit a transient submodule fetch error on attempt $attempt/3. Retrying after clearing partial submodule state...${ENDC}"
            rm -rf "$RISCV"/riscv-gnu-toolchain/newlib
            git -C "$RISCV"/riscv-gnu-toolchain submodule deinit -f --all >/dev/null 2>&1 || true
            git -C "$RISCV"/riscv-gnu-toolchain submodule sync --recursive >/dev/null 2>&1 || true
            git -C "$RISCV"/riscv-gnu-toolchain submodule update --init --recursive --depth 1 >/dev/null 2>&1 || true
            if [ "$attempt" -eq 3 ]; then
                echo -e "${ERROR_COLOR}RISC-V GNU Toolchain build failed after 3 attempts.${ENDC}"
                exit 1
            fi
        else
            echo -e "${ERROR_COLOR}RISC-V GNU Toolchain build failed with a non-network error.${ENDC}"
            exit 1
        fi
    done
    if [ "$clean" = true ]; then
